...
function Header() {
  ...
  const [mobileMenuTop, setMobileMenuTop] = useState(103)
  ...
  useLayoutEffect(() => {
    ...
    if (mobileMenuBodyRef.current) {
      mobileMenuBodyRef.current.scrollTop = 0
    }
  ...
  return (
    <header ref={headerRef} className={`header${isMobileMenuOpen ? ' header--menu-open' : ''}`}>
      <FavoritesPanel />
      ...
      {isMobileMenuOpen &&
        createPortal(
          ...
          >
            ...
            />
            <div
              className="mobile-menu-layer__panel"
              role="navigation"
